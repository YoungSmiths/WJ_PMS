    ...
    export default {
        ...
        modal: options => {
            if (!modalVm) {
                ...
                modalVm = new Vue({
                    el: container,
                    template: `<div class="modal exportModalProduce" v-if="!hidden">
                    ...
                    updated () {
                        return;
                        $('.exportModalProduce').validation();
                        if (this.hidden == false) {
                            $.ajax({
                                url: `${this.apiBase}/project/getGenPlatformInfo`,
                                type: 'GET',
                                contentType: 'application/json; charset=utf-8',
                                cache: false,
                                success: (json) => {
                                    var html = "<option value='{\"userName\":\"\",\"passWord\":\"\",\"groupName\":\"\",\"hostIp\":\"\"}'>选择模板</option>";
                                    for (var i = 0; i < json.data.length; i++) {
                                        html += "<option value='" + JSON.stringify(json.data[i]) + "'>" + json.data[i].userName + "/" + json.data[i].hostIp + "/" + json.data[i].groupName + "</option>"
                                    }
                                    $(".chooseGenplantform").html(html);
                                    $(".chooseGenplantform").change(function () {
                                        var getjson = JSON.parse(this.value);
                                        for (var o in getjson) {
                                            $("input[name=" + o + "]").val(getjson[o]);
                                        }
